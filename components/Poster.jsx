...
import { playTrack } from "../reducers/trackReducers";

const Poster = ({ track, chooseTrack }) => {
  // const [play, setPlay] = useRecoilState(playState);
  const { track: playingTrack } = useSelector((state) => state.playTrackSate);
  const { play } = useSelector((state) => state.playState);
  ...
  cursor-pointer hover:scale-105 hover:text-white/100 transition duration-200 ease-out group mx-auto"
    ...
    >
      <img
        src={track.albumUrl}
        alt={`${track.title} by ${track.artists}`}
